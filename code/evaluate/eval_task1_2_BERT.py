...
parser.add_argument("-_model1_path", help="path to model", dest="model1_path", default = '../../output/task1_BERT_18.pth')
parser.add_argument("-_model2_path", help="path to model", dest="model2_path", default = '../../output/task2_BERT_6.pth')
...
class BlessDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        word_pieces += (tokens_a + tokens_b + ["[SEP]"])
...
def get_predictions1(model, dataloader):
    ...
    d = {'sample_num': sample_num, 'y_true': y_true, 'pred_label': pred_label, 'y_pred': y_pred}
    ...
    print('task1 ROC AOC:', roc_auc)
    return predictions, roc_auc, correct, total, f1
...
model_fine.eval()
model_fine = model_fine.to(device)
_, roc_auc, correct, total, f1 = get_predictions1(model_fine, testloader1)
del model_fine
...
model_fine.eval()
model_fine = model_fine.to(device)
_, roc_auc, correct, total, f1 = get_predictions2(model_fine, testloader2)
del model_fine
...
testset_Hyper = hyperlexDataset(tokenizer=tokenizer)
BATCH_SIZE = 64
testloader_Hyper = DataLoader(testset_Hyper, batch_size=BATCH_SIZE, 
...
def hyper_predictions(model1, model2, dataloader):
...
#                 total += labels.size(0)
            ...
            for i in range(pred1.size()[0]):
                hyper6.append(label1s[i].item())
                hyper10.append(label2s[i].item())
                y_pred.append(logits1[i][1].item())
                pred_label.append(pred1[i].item())
...
model1_fine = BertForSequenceClassification.from_pretrained(PRETRAINED_MODEL_NAME, num_labels=NUM_LABELS, 
                                                            state_dict=state_dict)
model1_fine.eval()
model1_fine = model1_fine.to(device)

state_dict = torch.load(model2_path)
model2_fine = BertForSequenceClassification.from_pretrained(PRETRAINED_MODEL_NAME, num_labels=NUM_LABELS, 
                                                            state_dict=state_dict)
model2_fine.eval()
model2_fine = model2_fine.to(device)

_, acc = hyper_predictions(model1_fine, model2_fine, testloader_Hyper)
del model1_fine
del model2_fine
torch.cuda.empty_cache()
...
testset_Wordnet = wordnetDataset(tokenizer=tokenizer)
BATCH_SIZE = 64
testloader_Wordnet = DataLoader(testset_Wordnet, batch_size=BATCH_SIZE, 
                         collate_fn=create_mini_batch_wordnet, shuffle = False)


def wordnet_predictions(model1, model2, dataloader):
    ...
    with torch.no_grad():
        ...
        for data in (dataloader):
            ...
            for i in range(pred1.size()[0]):
                ...
                if pred1[i].item() and pred2[i].item():
...
_, acc = wordnet_predictions(model1_fine, model2_fine, testloader_Wordnet)
print('accuracy: ', acc)
del model1_fine
del model2_fine
...
net.from_nx(G)
net.show('../../output/wordnet_reconstruction_BERT.html')
